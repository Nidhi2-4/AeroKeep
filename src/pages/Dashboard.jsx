...
export default function Dashboard() {
  ...
  const cards = [
    ...
    { label: 'STOCK VALUE', value: formatCurrency(stats.totalValue), icon: 'payments', color: 'text-green-400', border: 'glow-border-amber', valueColor: 'text-green-400' },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {cards.map((card) => (
          <div key={card.label} className={`glass-panel ${card.border} p-6 rounded-xl transition-all duration-300 hover:scale-[1.02]`}>
      ...
      {lowStock.length > 0 && (
        <div className="mb-6 p-4 bg-amber-400/5 border border-amber-400/20 rounded-xl border-l-4 border-l-amber-400 flex items-center justify-between">
          ...
          <Link to="/inventory" className="font-label-sm text-[11px] text-amber-400 hover:text-amber-300 uppercase tracking-wider whitespace-nowrap ml-4">
            View Alerts →
          </Link>
        </div>
      )}
      ...
      <div className="glass-panel rounded-xl overflow-hidden">
        ...
        {loading ? (
          <div className="p-12 text-center text-on-surface-variant font-body-base">Loading...</div>
        ) : movements.length === 0 ? (
          <div className="p-12 text-center text-on-surface-variant font-body-base">No activity yet. Add products and adjust stock to see logs here.</div>
        ) : (
              ...
              <tbody>
                {movements.map((m) => (
                  <tr key={m.id} className="border-b border-white/[0.03] hover:bg-tertiary/[0.03] transition-colors">
                    ...
                    <td className="px-5 py-3 font-body-base text-[13px] text-on-surface-variant max-w-[180px] truncate">{m.reason}</td>
